 ...
 * Supports one-touch pan, and pinch-to-zoom.
 */

// this class is a copy from the original google/filament GestureDetector
// but setting the same gesture for orbit and pan with a switch
class GestureDetector(private val view: View, private val manipulator: Manipulator) {
    ...
    private var orbit = false;

    fun swapOrbitPan(orbit: Boolean) {
        this.orbit = orbit
    }
...
//                    (event.pointerCount != 2 && currentGesture == Gesture.PAN) ||
    ...
    private fun isPanGesture(): Boolean {
        return tentativePanEvents.size > kGestureConfidenceCount
    }
